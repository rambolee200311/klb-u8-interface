...
using KlbU8Vouch.GLVouch;
using KlbU8Vouch.DispatchList;

namespace BidlinkU8Interface.Controllers
{
    public class DispatchListController : ApiController
    {
        [Route("dispatchlist/add")]
        [HttpPost]
        public KlbU8Vouch.GLVouch.Result addDispatchList([FromBody]InMain inMain)
        {
            Log.AddInfo("DispatchListController.addDispatchList()", "Begin");
            KlbU8Vouch.GLVouch.Result result = new KlbU8Vouch.GLVouch.Result();
            Log.AddDebug("DispatchListController.addDispatchList()", "inData:" + inMain.ToJson());
            string AccID = "";//账套
            string User = "";//用户名
            string Password = "";//密码
            string Server = "";//服务器
            ...
            result = DispatchListEntity.addDBVouch(result, inMain, AccID, User, Password, Server);

            Log.AddInfo("DispatchListController.addDispatchList()", "outData:" + result.ToJson());
            Log.AddInfo("DispatchListController.addDispatchList()", "End");
            return result;
        }
    }
}
